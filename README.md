...
QgsRasterLayer是处理栅格数据的核心类，负责加载、显示渲染和操作栅格图层（如卫星影像、DEM 数据、遥感图像等）。  QgsRasterLayer对象通常由QgsProject管理生命周期，添加到项目后不需要手动删除。  QGIS的大部分API不是线程安全的，应避免在非主线程中操作栅格图层。  QgsRasterLayer支持多种栅格格式，具体取决于编译 QGIS 时启用的 GDAL 驱动。
